...
static void build_ui(void)
{
    ...
    lv_obj_set_style_text_letter_space(clock_label, 2, 0);
    lv_obj_set_style_transform_scale_x(clock_label, 320, 0);
    lv_obj_set_style_transform_scale_y(clock_label, 320, 0);
    ...
    lv_obj_set_style_pad_column(weather_card, 18, 0);
    ...
    lv_obj_set_style_pad_gap(weather_text, 4, 0);
    ...
    lv_label_set_text(temperature_label, "0°");
    ...
    lv_obj_set_style_text_letter_space(temperature_label, 1, 0);

    weather_caption = lv_label_create(weather_text);
    lv_label_set_text(weather_caption, "Waiting for weather data");
    ...
    lv_label_set_text(weather_hint, "Wind: 0 km/h | Sunrise: --");
...
void ui_set_weather(int temperature_c, int wind_kmh, const char *sunrise_iso8601, const char *icon_name)
{
    ...
    if (weather_caption != NULL)
    {
        lv_label_set_text(weather_caption, "Current conditions");
    }
    ...
    if (weather_hint != NULL)
    {
        lv_label_set_text_fmt(
            weather_hint,
            "Wind: %d km/h | Sunrise: %s",
            wind_kmh,
            sunrise_iso8601 != NULL ? sunrise_iso8601 : "--");
    }
}
